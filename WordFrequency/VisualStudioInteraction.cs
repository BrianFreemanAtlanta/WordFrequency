...
namespace WordFrequency
{
    ...
    internal static class VisualStudioInteraction
    {
        ...
        private static readonly string m_Title = "Word Frequency Utilization";
...
//        }
        ...
        public static async Task ShowExceptionAsync(Exception ex)
        {
            ...
            if (!CancelAllToken.IsCancellationRequested) {
                ...
                MessageBox.Show($"An error has occured during conversion - press Ctrl+C to copy the details: {ex}{versionMessageSuffix}",
                    m_Title, System.Windows.MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        {
            await ThreadHelper.JoinableTaskFactory.SwitchToMainThreadAsync(CancelAllToken);
            var span = await FirstSelectedSpanInCurrentViewPrivateAsync(serviceProvider, mustHaveFocus);
            await TaskScheduler.Default;
            return span;
        }
...
#pragma warning restore VSTHRD010 // Invoke single-threaded types on Main thread
        ...
        private static async Task<Span?> FirstSelectedSpanInCurrentViewPrivateAsync(
            IAsyncServiceProvider serviceProvider,
             bool mustHaveFocus)
        {
            var selection = await GetSelectionInCurrentViewAsync(serviceProvider, mustHaveFocus);
            return selection?.SelectedSpans.First().Span;
        }
        ...
        { 
            ThreadHelper.ThrowIfNotOnUIThread();
            //ThreadHelper.JoinableTaskFactory.SwitchToMainThreadAsync(CancelAllToken);
        ...
        public static void OutputWindowWrite(string paneMessage)
        {
            ThreadHelper.ThrowIfNotOnUIThread();
            // Create a tool window handle for the Output window.  
            DTE2 dte =Package.GetGlobalService(typeof(DTE)) as DTE2;
            var win = dte.ToolWindows.OutputWindow; //dte.Windows.Item(EnvDTE.Constants.vsWindowKindOutput);
            // Create handles to the Output window and its panes.  
            OutputWindowPane pane;
            try
            {
                pane = win.OutputWindowPanes.Item("WordCount");
            }
            catch
            {
                pane = win.OutputWindowPanes.Add("WordCount");
            }
            win.Parent.Activate();
            pane.OutputString(paneMessage);
        }

        public static void OutputWindowWriteLine(string paneMessage)
        {
            OutputWindowWrite( paneMessage + "\n");
        }
    }
}
